...
#define _first_(i, j) first[(i) * local_size + (j)]
#define _second_(i, j) second[(i) * local_size + (j)]
#define _multiply_(i, j) multiply[(i) * local_size + (j)]
            ...
            for (k = 0; k < local_size;) { //columns in first and rows in second
                if (k < local_size - 4) {
                    sum = sum + _first_(i, k)*_second_(k, j) 
                              + _first_(i, k+1)*_second_(k+1, j)
                              + _first_(i, k+2)*_second_(k+2, j)
                              + _first_(i, k+3)*_second_(k+3, j);
                    k += 4;
                } else {
                    sum = sum + _first_(i, k)*_second_(k, j);
                    k++;
